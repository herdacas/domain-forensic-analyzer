...
"""
SecurityTrails Client für Domain Forensic Analyzer
...
class SecurityTrailsClient:
    """
    Modern SecurityTrails Integration für Historical DNS Intelligence
    """
    
    def __init__(self):
        """Initialize SecurityTrails Client with secure API management"""
            ...
            if mx_response.status_code == 200:
                ...
                for record in mx_records:
                    historical_data['mx_records'].append({
                        'first_seen': record.get('first_seen'),
                        'last_seen': record.get('last_seen'),
                        'mail_servers': [val.get('hostname') for val in record.get('values', [])]
                    })
    ...
    def _categorize_subdomains(self, subdomains: list) -> Dict[str, list]:
        ...
        for subdomain in subdomains:
            ...
            if any(pattern in sub_lower for pattern in ['admin', 'panel', 'manage', 'control']):
                categories['admin'].append(subdomain)
            elif any(pattern in sub_lower for pattern in ['api', 'rest', 'graphql', 'webhook']):
                categories['api'].append(subdomain)
            elif any(pattern in sub_lower for pattern in ['dev', 'test', 'stage', 'debug']):
                categories['dev'].append(subdomain)
            elif any(pattern in sub_lower for pattern in ['mail', 'smtp', 'imap', 'pop']):
    ...
    def _create_intelligence_summary(self, domain_info: Dict, historical: Dict, subdomains: Dict) -> Dict[str, Any]:
        ...
        if len(historical.get('a_records', [])) > 0:
            ...
            if len(set(recent_ips)) > 5:
                summary['risk_indicators'].append('Multiple IP address changes detected')
